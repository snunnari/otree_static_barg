...
doc = """
Multilateral alternating-offer legislative bargaining with five-players as in the model by Baron, David and John Ferejon 1989 
...
class Constants(BaseConstants):
    name_in_url = 'barg_five'
    players_per_group = 5
    num_rounds = 50
    budget = 250 # number of tokens to divide among committee members
    veto = False  # set to "True" for game with one veto player, set to "False" for game without veto players
    q = 3  # number of positive votes required for passage (if veto = True, this includes the veto player)
    rec_prob_1 = 1/5 # recognition probability of group member 1; if veto = True, this is the veto player
    rec_prob_2 = 1/5 # recognition probability of group member 2
    rec_prob_3 = 1/5 # recognition probability of group member 3
    rec_prob_4 = 1/5 # recognition probability of group member 4
    rec_prob_5 = 1/5 # recognition probability of group member 5
        ...
        if self.round_number == 1:
            ...
            for g in groups:
                ...
                if rand_num_prop <= Constants.rec_prob_1:
                    ...
                    g.proposer = 2
                elif rand_num_prop > Constants.rec_prob_2 and rand_num_prop <= Constants.rec_prob_1 + Constants.rec_prob_2 + Constants.rec_prob_3:
                    g.proposer = 3
                elif rand_num_prop > Constants.rec_prob_3 and rand_num_prop <= Constants.rec_prob_1 + Constants.rec_prob_2 + Constants.rec_prob_3 + Constants.rec_prob_4:
                    g.proposer = 4
                else:
                    g.proposer = 5
                # g.proposer = np.random.randint(1, 6) # alternative way to randomly determine proposer with even probabilities
...
class Group(BaseGroup):
    ...
    chosen_offer_to_4 = models.FloatField()
    chosen_offer_to_5 = models.FloatField()
    allocation_to_1 = models.FloatField()
    allocation_to_2 = models.FloatField()
    allocation_to_3 = models.FloatField()
    allocation_to_4 = models.FloatField()
    allocation_to_5 = models.FloatField()
...
class Player(BasePlayer):
    ...
    offer_to_4 = models.FloatField(
        max=Constants.budget,
        min=0
    )
    offer_to_5 = models.FloatField(
        max=Constants.budget,
        min=0
    )
